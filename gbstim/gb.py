...
class GBCode:
    def __init__(
        ...
        self.M = [
            np.kron(permutation_matrix(l, i), galois.GF2.Identity(m))
        ...
        self.n = 2 * l * m
        self.k = (
            self.n - np.linalg.matrix_rank(self.Gx) - np.linalg.matrix_rank(self.Gz)
        )

        # Data Qubits
        self.data_1 = [
            DataQubit(
                f"d1@{x * m + y}", x * m + y, (2 * x + 1, 2 * y), (2 * x + 1, 2 * y)
            )
            for x in range(l)
            for y in range(m)
        ]
        ...
        for data in self.data_1 + self.data_2:
            self.device.device_array[data.global_coords[1], data.global_coords[0]] = (
                data
            )
        ...
        def get_ab(f, h):
            ...
            for i in a:
                for j in b:
                    alpha = self.M[i]
                    beta = self.M[j]
                    if mat2poly(alpha.T @ beta)[0] in mat2poly(
                        poly2mat(f) @ poly2mat(h)
                    ):
                        ab_idx.append((i, j))
                        break
                b.remove(j)
        ...
        def get_XZ(f: list[int] = None):
            ...
            g_polys = [
                np.where(BA_null[i, : (l * m)] == 1)[0] for i in range(BA_null.shape[0])
            ]
            h_polys = [
                np.where(BA_null[i, (l * m) :] == 1)[0] for i in range(BA_null.shape[0])
            ]
            ...
            for f in f_polys:
                for g, h in zip(g_polys, h_polys):
                    ...
                    if (
                        np.linalg.matrix_rank(X) == self.k
                        and np.linalg.matrix_rank(Z) == self.k
                    ):
                        if (
                            np.count_nonzero(X @ self.Gz.T) == 0
                            and np.count_nonzero(Z @ self.Gx.T) == 0
                        ):
                            return X, Z
        ...
        self.d = min(
            self.estimate_d(self.Gx, self.X, 1_000)[0],
            self.estimate_d(self.Gz, self.Z, 1_000)[0],
        )
        ...
        for i in range(self.Gx.shape[0]):
            ...
            for (x_pow, y_pow), A_term in zip(A_poly, A_terms):
                ...
                meas_data_1.append(
                    (calculate_periodicity(i, x_pow, y_pow), self.data_1[loc])
                )
            meas_data_2 = []
            for (x_pow, y_pow), B_term in zip(B_poly, B_terms):
                loc = np.where(B_term[i] == 1)[0][0]
                meas_data_2.append(
                    (calculate_periodicity(i, x_pow, y_pow), self.data_2[loc])
                )
            coords = (2 * (i // m) + 1, 2 * (i % m) + 1)
            meas_x = GBMeasureQubit(
                meas_data_1,
                meas_data_2,
                f"x@{i}",
                anc_name,
            ...
            for (x_pow, y_pow), B_term in zip(B_poly, B_terms):
                ...
                meas_data_1.append(
                    (
                        3 ^ calculate_periodicity(i, l - x_pow, m - y_pow),
                        self.data_1[loc],
                    )
                )
            meas_data_2 = []
    ...
    def estimate_d(self, H, O, T, **kwargs):
        ...
        for i in range(T):
            ...
            decoder = BpOsdDecoder(
                fake_check,
                error_channel=list((1e-3) * np.ones(fake_check.shape[1])),
                max_iter=50,
                osd_method="osd_0",
                **kwargs,
            )
    ...
    def move_atoms(self, atoms, shift):
        if not shift:
            return 0
        move_time = np.sqrt((6 * abs(shift[0]) * self.device.spacing) / 0.02) + np.sqrt(
            (6 * abs(shift[1]) * self.device.spacing) / 0.02
        )
        for atom in atoms:
            self.atom_pos[atom] = (
                self.atom_pos[atom][0] + shift[0],
                self.atom_pos[atom][1] + shift[1],
            )
    ...
    def schedule_movement(self):
        ...
        def gen_points(x_offset, y_offset, subgrid):
            ...
            if x_offset > 0 and y_offset > 0:
                yield (
                    3,
                    [x_target - 2 * spacing * self.l, y_target - 2 * spacing * self.m],
                )
        ...
        # Create distance matrix for TSP solver
            ...
            for j in range(num_points):
                if i != j:
                    ...
                    distance_matrix[i, j] = np.sqrt(
                        np.sum((pt_array[i] - pt_array[j]) ** 2)
                    )

        # Solve TSP using python-tsp
        permutation, _ = solve_tsp_dynamic_programming(distance_matrix)

        # Convert permutation to tour (skipping the first point which is the start/end)
        sol = [permutation[i] for i in range(1, len(permutation))]
        ...
        for pt_idx in sol:
            ms_perm.append(
                pts_dict[tuple(pt_array[pt_idx])]
                + len(self.b_poly) * 4
                + len(self.a_poly) * 4
            )
...
class CustomUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        breakpoint()
        if module == "sinter" and name == "_task_stats":
            return sinter._task_stats
